      ...
      <div className="content-card">
        ...
        <div className="card-content">
          ...
          <div className="table-responsive mt-4 mb-4">
            <h4 style={{ textAlign: 'center', color: '#ffd700', marginBottom: '1rem' }}>Demand Schedule</h4>
            <table className="table table-dark table-striped text-center" style={{ borderRadius: '10px', overflow: 'hidden' }}>
              <thead style={{ background: '#b8860b' }}>
                <tr>
                  <th>Price (₹)</th>
              ...
              <tbody>
                {DEMAND_DATA.map((row, index) => (
                  <tr key={index}>
                    <td style={{ verticalAlign: 'middle' }}>{row.price}</td>
                    <td style={{ verticalAlign: 'middle' }}>{row.quantity}</td>
                    <td style={{ color: index === 0 ? '#4da6ff' : (row.price < DEMAND_DATA[index - 1]?.price ? '#00ff00' : '#ff4444') }}>
                      {index === 0 ? 'Initial Point' : (row.price < DEMAND_DATA[index - 1].price ? 'Price Falls, Qty Rises' : 'Price Rises, Qty Falls')}
                    </td>
                  </tr>
                ))}
              </tbody>
